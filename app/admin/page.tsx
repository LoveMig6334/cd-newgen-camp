...
async function getStats() {
  ...
  const [{ count: eventsCount }, { count: applicantsCount }, { data: activeEvent }] = await Promise.all([
    supabase.from('events').select('*', { count: 'exact', head: true }),
    supabase.from('applications').select('*', { count: 'exact', head: true }),
    supabase.from('events').select('name, year').eq('is_active', true).limit(1).single(),
  ])
...
export default async function AdminDashboard() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: activeEvent ? `${activeEvent.name} (${activeEvent.year})` : 'ไม่มี',
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
