...
type EventOption = Pick<Event, 'id' | 'name' | 'year' | 'max_applicants' | 'is_active'>

function exportCSV(applicants: Application[], eventName: string) {
  const headers = ['ชื่อ', 'นามสกุล', 'อีเมล', 'โทรศัพท์', 'โรงเรียน', 'ระดับชั้น', 'วันที่สมัคร']
...
export default function ApplicantsViewer({ events }: { events: EventOption[] }) {
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex flex-col sm:flex-row gap-3 flex-1">
          <select
            ...
            className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 min-w-[220px]"
          >
      ...
      {selectedEvent && (
        <div className="mb-3 text-sm text-gray-600">
          ...
          <span className="font-semibold text-gray-900">{selectedEvent.max_applicants}</span> คน
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-400">กรุณาเลือกกิจกรรม</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-gray-600">นามสกุล</th>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-gray-600">โรงเรียน</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-600">ระดับชั้น</th>
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="text-center py-8 text-gray-400">
                      {applicants.length === 0 ? 'ยังไม่มีผู้สมัคร' : 'ไม่พบผลลัพธ์'}
                    </td>
                  </tr>
                ) : (
                  filtered.map((applicant, i) => (
                    <tr key={applicant.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-900">{applicant.first_name}</td>
                      <td className="px-4 py-3 text-gray-900">{applicant.last_name}</td>
                      <td className="px-4 py-3 text-gray-600">{applicant.email}</td>
                      <td className="px-4 py-3 text-gray-600">{applicant.phone}</td>
                      <td className="px-4 py-3 text-gray-600">{applicant.school}</td>
                      <td className="px-4 py-3 text-gray-600">{applicant.grade}</td>
                      <td className="px-4 py-3 text-gray-500 whitespace-nowrap">
                        {new Date(applicant.created_at).toLocaleDateString('th-TH', {
